...
export default async function Media({ searchParams }: PageParams) {
  ...
  const [{ data: mediaPage }, { data: totalCount }, { data: media }] = await Promise.all([
    ...
    sanityFetch({
      ...
      params: { search: search || null, start, end },
    }) as Promise<{ data: Media[] }>,
  ])
  ...
  return (
    <div>
      ...
      <div className="container mx-auto my-20 px-4 md:px-6">
        ...
        <div className="grid min-h-[180px] grid-cols-2 place-items-center gap-6 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
          {media.length === 0 ? (
